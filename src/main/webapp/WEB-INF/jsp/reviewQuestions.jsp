					...
					<div class="col-md-4 comment-area mrg-top">
					        <label>Old Answer</label><br>
							   ...
							   <label>Old Comment</label>
								...
								<input type="text"  class="form-control abc${status.count}" id="old_selected_option comment" name="answerdata[${status.count}].oldselectOptioncomment" value="${mvBean.selected_option_comment}" readonly>
								<br>
							...
							<label>New Answer</label><br>						
			            		<select name="answerdata[${status.count}].optionid" id="selectoptioncomment" required  class="form-control selectoptioncomment">
									...
									<option value="${mvBean1.optionId}#${mvBean1.option_comment}# ${mvBean1.option_points}#${status.count}">${mvBean1.options}</option>
									</c:forEach>
									</select>
					...
					<div class="clearfix"></div>
			   		...
			   		<div class="center">		    	
					    <input type="submit" class="btn btn-info  btn-gradient" value="Update">
					     <a href="<%=dashboardURL%>getMysQuestionnaire/${shopper_id}" class="btn btn-light waves-effect m-l-5">
                                                Cancel
                                            </a>
				  	</div>	
 ...
 // var old_selected_option=$("input[name=old_selected_option]").val();
  ...
  $(".selectoptioncomment").each(function(){
	  ...
	  $(this).on("change",function(){
		  ...
		  var count=status[3];
		  var old_selected_option=$(".abc"+count).val();
