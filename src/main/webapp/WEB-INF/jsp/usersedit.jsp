					...
					<div class="row">
						...
						<div class="col-lg-12">
							...
							<div class="card-box ">
								...
								<form method="POST" action="<%=dashboardURL%>updateUser" autocomplete="off">
									<div class="row">
									...
									<div class="form-group col-md-6">
										...
										<div class="form-group col-md-5">
											...
											<select id="user_type_id" name="user_type_id" required
												...
												<c:forEach var="uBean" items="${userTypeList}">
													<option value="${uBean.user_type_id}" <c:if test ="${user_type_id == uBean.user_type_id}">selected </c:if>>${uBean.user_type }</option>
												</c:forEach>
											</select>
										</div>
										...
										<div class="form-group col-md-6">
											...
											<select id="sk_role_id" name="sk_role_id" required
												...
												<c:forEach var="uBean" items="${roleList}">
													<option value="${uBean.sk_role_id}" <c:if test ="${sk_role_id == uBean.sk_role_id}">selected </c:if>>${uBean.role_name}</option>
												</c:forEach>
										...
										<div class="form-group col-md-6">
											...
											<select id="region_id" name="region_id" required
												...
												<c:choose>
                                     <c:when test="${regionId=='All'}">
                                      <option value="${regionId}">${regionId}</option> 
                                      </c:when>
                                      <c:when test="${regionId==''}">
                               ...
                               <option value="${regionId}">${region_name}</option> 
                                      ...
                                      <c:when test="${dealerId==null || dealerId.isEmpty()}">
                                      <option value="">Select Dealer</option> 
                                      </c:when>
                               <c:otherwise>
                               	<c:forEach var="uBean" items="${dealerList}">
								<option value="${uBean.dealer_id}" <c:if test ="${dealerId == uBean.dealer_id}">selected </c:if>>${uBean.dealer_name}</option>
								</c:forEach> 
                               </c:otherwise>
                              </c:choose> 
                              </select>
										...
										<div class="form-group col-md-6">
											<label for="Address">First Name<span
												...
												placeholder="Enter First Name" value="${first_name}" required data-parsley-maxlength="100">
												<input type="hidden" 
												class="form-control" id="sk_user_id" name="sk_user_id"
												data-parsley-pattern=""
												placeholder="Enter First Name" value="${sk_user_id}">
										</div>
										<div class="form-group col-md-6">
											<label for="Address">Last Name<span
												...
												data-parsley-pattern="/^[a-zA-Z ]*$/"
												placeholder="Enter Last Name"  value="${last_name}" required data-parsley-maxlength="100">
										</div>
										<div class="form-group col-md-6">
									...
									<div class="form-group text-right m-b-0">
										<button class="btn btn-gradient waves-effect waves-light"
										  ...
										  <a href="<%=dashboardURL%>usersView"><button type="button" class="btn btn-light waves-effect m-l-5">
                                        Cancel
                                    </button></a>
...
<div id="login-modal" class="modal fade" tabindex="-1" role="dialog"
	...
	<script
	 ...
	 <script type="text/javascript">
	  ...
	  var userid= $("#sk_user_id").val();
		 $.ajax({
             url: "<%=dashboardURL%>getUserMailExisitanceByuserid",
             type: "GET", 
