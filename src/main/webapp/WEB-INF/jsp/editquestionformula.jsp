        ...
        <!-- End Navigation Bar-->
            ...
            <div class="content-page">
                ...
                <div class="content">
					...
					<form method="POST" action="<%=dashboardURL%>updateFormula/${qid}">
                        <div class="row">
                            <div class="col-lg-12">
		                        ...
		                        <button type="button" class="add_field_button3 btn btn-info">+</button>                  
	                            
	                             <c:forEach var="qBeans" items="${formulacount}" varStatus="status1">
                                <div class="card formulascount" id="formul" >
                              	  <div class="card-header" role="tab" id="headingOne0">
                               		 <h6 class="mb-0 mt-0">
                               			 <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne${status1.count}" class="text-dark" aria-expanded="true" aria-controls="collapseOneundefined">Add Formula ${status1.count}</a></h6>
                               	</div>
                               
                               	
                                	<div id="collapseOne${status1.count}" class="collapse show" role="tabpanel" aria-labelledby="headingOne">
                                	<c:forEach var="qBean" items="${subsetquestionsList}" varStatus="status">
                                		
                                		<div class="card-body formulae" id="formulae1">
                                			<div class="row">
                                				<div class="form-group col-md-9">
                                				
                                				
                                				<div class="checkbox1">
                                				  	<input id="checkbox0" name="formula[${status1.index}].formuladata[${status.count}].sk_subquestion_id" class="checkbox0" value="${qBean.sk_subquestion_id }" type="checkbox"  <c:forTokens items="${qBeans.sk_subquestion_id }" delims="," var="sid"> <c:if test = "${qBean.sk_subquestion_id == sid }">checked </c:if></c:forTokens> >
                                				 <label for="checkbox0">${qBean.subQuestion }</label>
                                				 
                               						</div>
                               					</div>
                               					
                               					<div class="form-group col-md-3">
                               						<select name="formula[${status1.index}].formuladata[${status.count}].sk_answer_id" class="form-control" id="formulaetype">
                                						<option value="">Select </option>
						                                <c:forEach  var="count" items="${optionsList.get(status.index)}">
						                               
    														<option value="${count.sk_answer_id }"  <c:forTokens items="${qBeans.sk_answer_id }" delims="," var="aid">  <c:if test = "${count.sk_answer_id == aid }">selected </c:if></c:forTokens>  >${count.answer }</option>
                                      ...
                                      <input type="hidden" name="formula[${status1.index}].formulaCount" value="f${status1.index}">
                                  <div class="pull-right  row">
                                <div class="form-group col-md-10"><label for="emailAddress">Result</label><input type="text" name="formula[${status1.index}].formulaFinalResult" parsley-trigger="change" value="${qBeans.formulaFinalResult }" placeholder="Enter Result" class="form-control" id="result"></div></div>
                                </div>
                                </div>
                               </c:forEach>
                               </div>
 ...
 --%>        <script src="<%=UI_PATH%>/plugins/autoNumeric/autoNumeric.js" type="text/javascript"></script>
        ...
        <script type="text/javascript">
            ...
            $(".selectedcheck").each(function(){
            	var selecteformula=$(this).attr("data-formulaid");
            	var formula=$(this).attr("data-formulaid1");
            	if(selecteformula==formula){
        ...
        $(add_button).on("click",function(e){
            ...
            if(x < max_fields){ //max input box allowed
                ...
                $(wrapper).append('<div class="card" id="formul" >'
                       +' <div class="card-header" role="tab" id="headingOne0">'
                       +'   <h6 class="mb-0 mt-0">'
                       +'  <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne'+y+'" class="text-dark" aria-expanded="true" aria-controls="collapseOneundefined">Add Formula '+y+'</a></h6>'
                       +'  </div><div id="collapseOne'+y+'" class="collapse" role="tabpanel" aria-labelledby="headingOne">'
                       +'  <c:forEach var="qBean" items="${subsetquestionsList}" varStatus="status">'
