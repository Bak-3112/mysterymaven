       ...
       <div class="content-page">
                ...
                <!-- end row -->
				  ...
				  <div class="row" >
                       ...
                       <%--     
						<table id="tblCustomers1" class="table table-striped table-bordered" cellspacing="0" width="100%" style="display:none">
                          <thead>
                                <tr>
                            		 <th>S.No</th>
                                    <th>Survey ID</th>
                                    <th>Proto Survey Id</th>
                                    <th>Wave</th>
                                    <th>Scenario</th>
                                    <th>Location Id</th>
                                    <th>Location</th>
                                    ...
                                    <th>Model</th>
                                    <th>Head Date</th>
                                    <th>Head Start Time</th>
                                    <th>Head Out Time</th>
                                    <th>[C103071] COMMENT: 0.1.1 Used Name:</th>
                                    <th>[C103072] COMMENT: 0.1.2 Used Telephone Number:</th>
                                    <th>[C103073] COMMENT: 0.1.3 Used E-Mail Address:</th>
                                    <th>[C103260] COMMENT: Model</th>
                                    <th>[C103261] COMMENT: Year of Construction:</th> 
                                    <c:forEach var="mvBean" items="${MSNResult}">
                                 <th>[Q${mBean.question_code}] ${mvBean.standard_number} ${mvBean.mainquestion}</th>
                                   <th>[Q${mvBean.question_code}]  Comment</th>   
                                 <c:forTokens var="token" items="${mvBean.subquestion}" delims="#">
                                    <th>[Q${mvBean.question_code}] ${mvBean.standard_number} <c:out value="${token}"/></th>
                                  <th>[Q${mvBean.question_code}]   Comment</th> 
                                    </c:forTokens>   
                                    </c:forEach> 
                                </tr>
                                ...
                                <tbody>
                                 <c:forEach var="mvBean" items="${allMSResultAns}" varStatus="status">
                                <tr>
                                <td>${status.index+1}</td>
                                <td> ${mvBean.sk_shopper_id} </td>
                                 <td></td>
                                 <td>${mvBean.quarter}</td> 
                                 <td>${mvBean.mode_of_contact}</td>
                                 <td>${mvBean.outlet_id}</td> 
                                 <td>${mvBean.dealer_name}</td>
                                 <td>${mvBean.address}</td> 
                                 <td>${mvBean.outlet_name}</td>  
                                 <td>${mvBean.pincode}</td>
                                 <td>${mvBean.brand_name}</td> 
                                 <td>${mvBean.shoppers_link_shoppersCarModel}</td>
                                 <td>${mvBean.visit_date}</td>
                                 <td>${mvBean.start_time}</td>
                                 <td>${mvBean.end_time}</td>
                                 <td>${mvBean.shopper_link_name}</td>
                                 <td>${mvBean.shopper_link_contact}</td>
                                 <td>${mvBean.shopper_link_email}</td> 
                                 <td>${mvBean.shoppers_car_owned}</td> 
                                 <td>${mvBean.shoppers_car_yop}</td> 
                               <c:forEach var="count" items="${subQnList1.get(status.index)}" varStatus="loop">
                                    <td> ${count.answer}</td>
                                     <td> ${count.main_ques_comment}</td>   
                                   <c:forTokens var="token" items="${count.subQuestionAnswer}" delims="#">
                                  <c:forTokens var="token1" items="${token}" delims="$$"> 
                             ...
                             <a href="<%=dashboardURL%>msmresultdownload" target="_blank" class="btn btn-secondary buttons-excel buttons-html5 download">Excel</a>
                        
                            <table id="tblCustomers" class="table table-striped table-bordered" cellspacing="0" width="100%">
                          <thead>
                                <tr>
                            		 <th>S.No</th>
                                     <th>Survey ID</th>
                                    ...
                                    <th>[C103072] COMMENT: 0.1.2 Used Telephone Number:</th>
                                    <th>[C103073] COMMENT: 0.1.3 Used E-Mail Address:</th>
                                      <th> [C103259] COMMENT: Brand:</th>
                                    <th>[C103260] COMMENT: Model:</th>
                                    <th>[C103261] COMMENT: Year of Construction:</th> 
                                     <th> [Q103075] 0.2.1 Brand:</th>
                                    <th>[Q103076] 0.2.2 Model:</th>
                                    <th>Gender:</th>
                                    <th>[C100676] COMMENT: 0.8 Calendar week of visit:</th> 
                                    <c:forEach var="mvBean" items="${MSNResult}">
                                 <th>[Q${mvBean.question_code}] ${mvBean.standard_number} ${mvBean.mainquestion}</th>
                                    
                                 <c:forTokens var="token" items="${mvBean.subquestion}" delims="#" varStatus="status1">
                                    <th>[Q${mvBean.question_code}A${status1.count}] A${status1.count}: ${mvBean.standard_number} <c:out value="${token}"/></th>
                                  <th>[C${mvBean.question_code}C${status1.count}] C${status1.count}: ${mvBean.standard_number} Comment</th> 
                                    </c:forTokens>   
                                     <th>[C${mvBean.question_code}]  Comment </th> 
                                    </c:forEach> 
                                </tr>
                                </thead>
                                <tbody>
                                 <c:forEach var="mvBean" items="${MSResultAns}" varStatus="status">
                                <tr>
                                <td>${status.index+1}</td>
                                 <td> ${mvBean.sk_shopper_id} </td>
                                  <td></td> 
                                 ...
                                 <td>${mvBean.shopper_link_contact}</td>
                                 <td>${mvBean.shopper_link_email}</td> 
                                 <td>${mvBean.shoppers_link_shoppersCarBrand}</td> 
                                  <td>${mvBean.shoppers_link_shoppersCarModel}</td> 
                                 <td>${mvBean.shoppers_car_yop}</td> 
                                  <td>${mvBean.brand_name}</td> 
                                  <td>${mvBean.model_name}</td> 
                                   <td>${mvBean.shopper_link_gender}</td> 
                                  <td>${mvBean.weekday}</td>  
                               <c:forEach var="mvBeans" items="${mvBean.getMSNResultAns}" varStatus="loop">
                                    <td> ${mvBeans.answer}<%-- ${mvBeans.standard_number} --%></td>
                                   <c:forTokens var="token" items="${mvBeans.subQuestionAnswer}" delims="#">
                                  <c:forTokens var="token1" items="${token}" delims="$$"> 
                                 <td><c:out value="${token1}"/> <%-- <c:out value="${mvBeans.standard_number}"/> --%></td> 
                                   </c:forTokens> 
                                   </c:forTokens> 
                                    <td> ${mvBeans.main_ques_comment}</td>   
                                     </c:forEach>
                                    </tr>
                                    </c:forEach>
                                </tbody>
...
var brand_id=$('#brand_id').val();
 function getIds(){
    ...
    var mode=$('#mode').val();
    $.ajax({
          url: "<%=dashboardURL%>getmsmresultcountIds",
          type: "GET", 
...
$('.Next').on('click', function(){
	...
	window.location.href ="/msmresult/"+report_type+"/"+year+"/"+month+"/"+region_id+"/"+sk_dealer_id+"/"+sk_outlet_id+"/"+brand_id+"/"+i;
  ...
  $( function() {
    $( ".datepicker" ).datepicker({
        ...
        $(".download").click(function(){
        	...
        	var sk_dealer_id= $("#sk_dealer_id").val();
        	var sk_outlet_id= $("#sk_outlet_id").val();
        	$(this).attr('href','<%=dashboardURL%>msmdownload/'+year+'/'+month+'/'+region_id+'/'+sk_dealer_id+'/'+sk_outlet_id+'/'+brand_id+'');
        });
