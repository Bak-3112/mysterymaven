       ...
       <div class="content-page">
                ...
                <div class="content">
                    ...
                    <div class="col-sm-12">
                        <div class="page-title-box">
                            ...
                            <h4 class="page-title">Create Weightage</h4>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-6">
                        ...
                        <div class="card-box ">
                            ...
                            <form method="POST" action="<%=dashboardURL%>addWeightage" enctype="multipart/form-data" autocomplete="off">
                          <label class="highlight1">Weightage can not exceed value 1. Kindly revise it.<span class="highlight">(*)</span></label>
                              <div class="form-group col-md-12">
										...
										<div class="form-group col-md-12">
                                    <label for="state_name">Section<span class="text-danger">*</span></label>
									<select id="section_id" name="section_id" required class="form-control select2 form-control select2-multiple" multiple data-placeholder="Choose Section.."  data-parsley-errors-container="#chke1">
                                ...
                                <div class="form-group col-md-12">
                                    <label for="state_name">Weightage(in decimal)<span class="text-danger">*</span></label>
                                    <input type="text" required class="form-control"  id="weightage" name="weightage" placeholder="Enter Wieightage" data-parsley-pattern="^[0-9]*\.[0-9]{2}$"  data-parsley-range="[0.01, 1]"><br>
                                </div>
                     ...
                     <div class="col-md-6">
                        <div class="card-box table-responsive">
                            ...
                            <ul class="nav nav-tabs">
                                ...
                                <tbody>
                              <c:forEach var="dbBean" items="${weightageList}"> 
                                <tr>
                                    <td>${dbBean.year}</td>
                                    <td>
                                    <c:forTokens items="${dbBean.section_name }" delims="," var="section_name">
                                       <p>${section_name}</p>
                                       </c:forTokens>
                                       </td>
                                    <td>${dbBean.weightage }</td>
                                    <td><a href="<%=dashboardURL%>editWeightage/${dbBean.sk_id}" class="fa fa-edit"></a>&nbsp;&nbsp;&nbsp;<a href="#"  class="fa fa-trash" data-id="${dbBean.sk_id}" data-toggle="modal" data-target="#myModal"></a></td>
                                </tr>
                               </c:forEach> 
                                </tbody>
                            </table>
                                ...
                                <tbody>
                               <c:forEach var="dbBean" items="${inactiveWeightageList}"> 
                                <tr>
                                    <td>${dbBean.year}</td>
                                     <td>
                                    <c:forTokens items="${dbBean.section_name }" delims="," var="section_name">
                                       ...
                                       <td>${dbBean.weightage }</td>
                                    <td><a href="#" data-restoreid="${dbBean.sk_id}" data-toggle="modal" data-target="#myModal1" title="Restore the Weightage" class="mdi mdi-backup-restore"></a></td>
                                </tr>
                                 </c:forEach> 
                                </tbody>
                            </table>
        ...
        <script type="text/javascript">
            ...
            $(".fa-trash").on("click",function(){
            	...
            	var url="<%=dashboardURL%>deleteWeightage/"+value;
            	$(".btn-edit").attr("href",url);
            });
            
            $(".mdi-backup-restore").on("click",function(){
            	var value=$(this).attr("data-restoreid");
            	var url="<%=dashboardURL%>restoreWeightage/"+value;
	...
	jQuery('#state_name').keyup(function() 
	{
      ...
      jQuery('#state_name').val(spart.join(" "));
        ...
        $(".btnsubmit").on("click",function(){
        	  $('form').parsley();
          	if($("#stateerrormsg").text()=="Section   already exist for the year...!"){
          		 $("#testbox").prop('required',true);
          		 setTimeout(function(){
             			var id=$("#testbox").attr("data-parsley-id");
             			  $("#parsley-id-"+id+"").hide();
             		}, 10);
          	}else	if($("#stateerrormsg").text()=="Section  in Inactive tab ,Please restore ...!"){
          		 $("#testbox").prop('required',true);
          		 setTimeout(function(){
             			var id=$("#testbox").attr("data-parsley-id");
             			  $("#parsley-id-"+id+"").hide();
    	...
    	$('#section_id').change(function(e) {
    		var section_id1= $("#section_id").select2("val");
        	var section_id=section_id1.toString();
        	//alert(section_id);
        	var year = $("#year").val();
    		
        	 $.ajax({
                 url: "<%=dashboardURL%>getWeightageExistance",
                 type: "GET", 
                   data: { 'section_id': section_id,'year':year },
                 success: function(response)
                 {
                	 var msg=response.message;
                   	 var statusmessage=response.status;
    					   if(msg=="sectionexistforthatyear" && statusmessage=="active"){
    						   $("#stateerrormsg").text("Section   already exist for the year...!")
    					   }else if(msg=="sectionnotexistforthatyear" && statusmessage=="inactive"){
    						   $("#stateerrormsg").text("Section in Inactive tab ,Please restore ...!")
    					   }else{
    					var id=$("#testbox").attr("data-parsley-id");
