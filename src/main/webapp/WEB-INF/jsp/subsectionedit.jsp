...
display: none;
    ...
    <div id="wrapper">
       ...
       <div class="content-page">
                ...
                <div class="content">
                    ...
                    <div class="col-sm-12">
                        <div class="page-title-box">
                            ...
                            <h4 class="page-title">Edit SubSection </h4>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-6">
                        ...
                        <div class="card-box ">
                            <form method="POST" action="<%=dashboardURL%>updatesubSection"  autocomplete="off">
                                <div class="form-group col-md-12">
                                    <label for="state_name">Section Name <span class="text-danger">*</span></label>
                                   <select id="sk_section_id" name="sk_section_id" required class="form-control"  >
                                       <c:forEach var="dbBean" items="${sectionList}">
                                     <option value="${dbBean.sk_section_id }" <c:if test = "${sectionId == dbBean.sk_section_id }">selected </c:if>>${dbBean.section_name }</option>
                                     </c:forEach> 
                                     </select>      
                                </div>
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <input type="text" required class="form-control" onkeyup="exist(this.value)" id="subsection_name" name="subsection_name" value="${subsectionName }" placeholder="Enter Sub Section Name" data-parsley-pattern="^[a-zA-Z ]+$"  data-parsley-maxlength="100">
                                     <input type="hidden" required class="form-control" id="sk_subsection_id" name="sk_subsection_id" value="${subsectionId }">
                                ...
                                <div class="form-group text-right m-b-0">
                                    ...
                                    <a href="<%=dashboardURL%>addsubSection"><button type="button" class="btn btn-light waves-effect m-l-5">
                                        Cancel
                                    </button></a>
        ...
        <script type="text/javascript">
            ...
            $(".fa-trash").on("click",function(){
            	...
            	var url="<%=dashboardURL%>deleteSubSection/"+value;
            	$(".btn-edit").attr("href",url);
            });
            
            $(".mdi-backup-restore").on("click",function(){
            	var value=$(this).attr("data-restoreid");
            	var url="<%=dashboardURL%>restoreSubSection/"+value;
	...
	jQuery('#subsection_name').keyup(function() 
	{
      ...
      jQuery('#subsection_name').val(spart.join(" "));
        ...
        $("#sk_section_id").on("change",function(){
        	var sectionId=$("#sk_section_id").val();
         	 var subsectionId=$("#sk_subsection_id").val();
         	 var subsection_name=$("#subsection_name").val();
        	exist(subsection_name);
        ...
        $(".btnsubmit").on("click",function(){
        	  $('form').parsley();
          	if($("#sectionerrormsg").text()=="Sub Section is  already exist ...!"){
          		 $("#testbox").prop('required',true);
          		 setTimeout(function(){
             			var id=$("#testbox").attr("data-parsley-id");
             			  $("#parsley-id-"+id+"").hide();
             		}, 10);
          	}else	if($("#sectionerrormsg").text()=="Sub Section is in Inactive tab ,Please restore ...!"){
          		 $("#testbox").prop('required',true);
          		 setTimeout(function(){
             			var id=$("#testbox").attr("data-parsley-id");
             			  $("#parsley-id-"+id+"").hide();
      ...
      <!-- state existeor not -->
        <script type="text/javascript">
        function exist(subSectionName){
      	  var sectionId=$("#sk_section_id").val();
      	 var subsectionId=$("#sk_subsection_id").val();
        	 $.ajax({
                 url: "<%=dashboardURL%>getSubSectionExistanceById",
                 type: "GET", 
                   data: { 'subSectionName': subSectionName,'sectionId':sectionId,'subsectionId':subsectionId },
                 success: function(response)
                             {
               	  var msg=response.message;
               	 var statusmessage=response.status;
               
  					   if(msg=="subsectionexist" && statusmessage=="active"){
  							//alert(statusmessage)
  						   $("#sectionerrormsg").text("Sub Section is  already exist ...!")
  					   }else if(msg=="subsectionexist" && statusmessage=="inactive"){
  						   $("#sectionerrormsg").text("Sub Section is in Inactive tab ,Please restore ...!")
  					   }else{
  					var id=$("#testbox").attr("data-parsley-id");
